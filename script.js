...
function productMoneyUpdater(productPriceId, itemInputVale, productPrice) {
  document.getElementById(productPriceId).innerText =
    itemInputVale * parseInt(productPrice);
}
// checkout working area
function checkOutPrizeUpdater() {
  let phoneTotalPrice =
    parseInt(document.getElementById("phonePrize").innerText) +
    parseInt(document.getElementById("casePrize").innerText);
  document.getElementById("subTotal").innerText = phoneTotalPrice;
  // taxCalculator
  const taxValue = (5 / 100).toFixed(2);
  let generatedTaxValue = parseFloat(
    (
      taxValue * parseInt(document.getElementById("subTotal").innerText)
    ).toFixed(2)
  );
  ...
  document.getElementById("totalWithTax").innerText =
    phoneTotalPrice + generatedTaxValue;
}
...
function itemsRemove(
  ...
  document
    .getElementById(itemRemoveIdName)
